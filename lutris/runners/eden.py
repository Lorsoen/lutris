...
class eden(Runner):
    ...
    # Eden currently uses an AppImage, no need for the runtime.
    system_options_override = [{"option": "disable_runtime", "default": True}]

    @property
    def eden_data_dir(self):
        """Return dir where Eden files lie."""
        candidates = ("~/.local/share/eden",)
    ...
    def _update_key(self, key_type):
        ...
        eden_data_dir = self.eden_data_dir
        if not eden_data_dir:
            logger.error("Eden data dir not set")
            return
        if key_type == "prod_keys":
            key_loc = os.path.join(eden_data_dir, "keys/prod.keys")
        elif key_type == "title_keys":
            key_loc = os.path.join(eden_data_dir, "keys/title.keys")
        else:
